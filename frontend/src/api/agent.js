...
export async function disconnectChannel(channel) {
  ...
  if (channel === 'email') {
    return apiCall('/api/notice/email/disconnect', { method: 'PATCH' })
  }
  return apiCall(`/api/notice/settings/${userEmail}`, { method: 'DELETE' })
}

export async function connectEmail() {
  return apiCall('/api/notice/email/connect', { method: 'PATCH' })
}
...
export async function connectChannel(channel) {
  ...
  // email 등 OAuth가 없는 기본 채널의 가상 연결
  return { ok: true, channel, connected: true }
}
