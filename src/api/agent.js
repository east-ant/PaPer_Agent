...
export async function disconnectChannel(channel) {
  return apiCall(`/api/notice/disconnect/${channel}`, { method: 'PATCH' })
}

/** 테스트 알림 전송 (채널 공통) */
export async function testChannel(channel) {
  if (channel === 'discord') return testDiscord()
  return apiCall(`/api/notice/test/${channel}`, { method: 'POST' })
}

/** 알림 채널 연결 (Discord는 OAuth, 나머지는 직접 연결) */
export async function connectChannel(channel) {
  if (channel === 'discord') return connectDiscord()
  return apiCall(`/api/notice/connect/${channel}`, { method: 'PATCH' })
}
